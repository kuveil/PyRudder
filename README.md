...
当前版本：`0.1.0-alpha.8`，适用于 Windows 10/11 x64。这是未签名的公开预发布版本，请先在开发环境中使用。安装后只有 CLI，没有桌面应用、托盘程序或常驻服务，也不需要终端初始化脚本。
...
打开 `pyrudder-…-Setup.exe`，选择界面语言和新的空目录，例如 `D:\Tools\PyRudder`。
...
系统 PATH 对本机所有用户及管理员终端生效，而安装目录仍由安装账户管理。这会带来跨用户命令解析风险；请仅在受信任的单用户开发机上启用此选项。自动配置失败或取消授权时，安装器会明确报告安装未完整成功；请先卸载，再重新安装。
...
本版本不支持覆盖升级、自动迁移或多份安装包并存。使用过旧版时先卸载，备份需要保留的数据，再选择新的空目录安装并重新登记已有 Python。
...
- 当前支持 Windows 10/11 x64、PowerShell/CMD 和稳定版标准 CPython x64；不接管 `py.exe`，不支持系统 Python 自动回退、ARM64/x86、Git Bash、PyPy、预发布或自由线程 Python。网络目录、重解析点目录、自动更新和安装目录迁移不在支持范围内。运行时及其脚本必须来自你信任的来源。
