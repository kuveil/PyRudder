...
2. Create `release/<version>` from the exact reviewed commit only when ready to publish: use `develop` for ordinary releases, or `hotfix/*` for an urgent release without unfinished features. The branch version must exactly match the Cargo workspace version; for example, the next prerelease could use `release/0.1.0-alpha.9`. **Pushing triggers public release automatically, without a separate publication approval.**
